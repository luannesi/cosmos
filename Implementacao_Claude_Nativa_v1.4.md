# Implementação Claude-Nativa v1.4 — CHAOS v2.6 + ORDER v2.6

**Status:** Especificação de implantação (Perfil A)
**Versão:** 1.4 (supersede 1.3)
**Data:** 2026-09-21
**Substitui:** Cenário E — Implementação GitHub-First v1.0 (descartado; ver §0)
**Contratos que implementa:** CHAOS v2.6 (*Capture · Harvest · Atomize · Organize · Synthesize*) · ORDER v2.6 (*Orchestrated Runtime for Distributed Execution & Reasoning*)
**Sistema:** este binding, com as duas camadas, constitui o **COSMOS** (CHAOS §1.0.1). O binding não tem nome próprio de propósito — é o membro substituível do trio.
...
## 0.0 Changelog v1.3 → v1.4 (o spike respondeu)

O spike de identidade que a v1.3 listava como **bloqueante** na Fase 0 foi
executado em 21/09/2026. Ele não passou. O binding muda em consequência.

**O que o spike mediu.** Seis commits de uma sessão Claude Code na nuvem sobre
um repositório descartável, lidos depois no histórico local. Resultado completo
em CHAOS §0.0. Em uma linha: **a sessão escolhe livremente autor e committer**
(`git config`, edição de `.git/config`), e **não consegue assinar** (`commit -S`
relatado como bem-sucedido, nenhuma assinatura gravada).

**O que isso faz com este documento.** A v1.3 dizia, na linha do spike: "se
falhar, a raiz de confiança sai do repositório e passa a ser o worker local
assinando aprovações com chave que só existe na máquina". Falhou. Essa frase
condicional é agora o desenho.

| Antes (v1.3) | Agora (v1.4) |
|---|---|
| Spike bloqueante antes de tudo | **Concluído.** A linha vira registro de resultado, com data e veredito |
| Worker local = componente de capacidade, entregue na Fase 1 | **Componente de segurança**, entregue na Fase 0, junto com as chaves |
| A4 = commit humano sem trailer | A4 = commit com **assinatura verificável de chave `human:*`** |
| Onboarding cria identidades Git por executor | Onboarding cria identidades **e chaves de assinatura**, e registra a primeira chave humana no `allowed_signers` |
| Fase 3 entrega aprovações | Fase 3 **depende** da Fase 0 ter entregue chave e worker; sem eles não há A4 possível |

**Outras mudanças:**

- §3.1 ganha a geração de chaves e o registro da primeira chave humana — que é
  o único momento em que `allowed_signers` é escrito sem assinatura prévia, por
  necessidade lógica (não há chave ainda), e por isso acontece no `chaos init`,
  fora do alcance de qualquer agente.
- §4.4: `metadata/registries/allowed_signers` como protected path com
  justificativa própria — é a raiz de confiança, não mais um arquivo de
  configuração entre outros.
- §9.1: a tabela de enforcement ganha uma quarta camada, a verificação de
  assinatura, que é a única que não depende de runtime, de remoto nem de fluxo
  de PR.
- §15: chave privada nunca no repositório, em nenhuma classe; frase-secreta
  obrigatória na chave humana.
- §16: dois riscos novos (11 e 12) e um risco fechado (o 7 muda de natureza).
- `order/policies/worker.yaml` ganha `signature_mode` (padrão `always`): quando
  o worker grava a marca de procedência. A chave privada dele vem da variável
  de ambiente `CHAOS_WORKER_SIGNING_KEY`, nunca de caminho versionado.
- `chaos init` e `chaos bootstrap sync` configuram o Git **local** do
  repositório (`gpg.format=ssh`, `gpg.ssh.allowedSignersFile`). Não é
  segurança — as ferramentas passam `-c` em toda verificação justamente para
  não depender disso — é para o humano: sem essa configuração,
  `git log --show-signature` mostra `N` num commit assinado, que é a leitura
  mais enganosa possível.
- §18.3: o pacote portátil não contém chave alguma — o texto já dizia
  "nenhuma credencial", e agora diz por que isso ficou mais grave.

**Achado colateral do spike, fora do escopo dele.** O `git clone` numa
instalação limpa do Git for Windows falhou por ausência de `credential.helper`.
O Onboarding pressupunha que o Git da máquina alcança o remoto, e numa máquina
nova ele não alcança. §3.1 ganha um passo explícito e verificável de
autenticação Git **antes** de qualquer `clone` ou `push`. Contato com a
realidade encontrou o que a releitura não encontrava — de novo.
...
| Identidades Git: a do humano e uma **por executor** (app do Claude Code para a nuvem; conta de serviço ou deploy key para o worker) — nunca reutilizar a credencial humana no worker. **Atribuição e quota apenas; nenhuma decisão de autorização consulta este registro** (CHAOS §17.1) | `metadata/registries/executors.yaml`; credenciais fora do repo |
| **Autenticação Git da máquina, verificada antes de qualquer `clone` ou `push`** — `git config --global credential.helper` deve responder, e um `clone` de teste do remoto deve concluir. Numa instalação limpa do Git for Windows não há helper configurado e a autenticação por senha não existe mais no GitHub: sem este passo, o primeiro `chaos push` falha com erro de autenticação que não parece erro de configuração | verificação do Onboarding; `TEC-` se algo tiver de ser instalado |
| **Chave de assinatura humana** — `ssh-keygen -t ed25519`, **com frase-secreta obrigatória**; a chave privada fica na máquina e nunca entra em repositório algum | `metadata/registries/allowed_signers` (pública); privada fora do repo |
| **Chave de assinatura do worker** — par próprio, sem frase-secreta, principal distinto do humano; prova procedência (veio desta máquina), nunca consentimento (ORDER §18) | `metadata/registries/allowed_signers` (pública); privada no diretório do worker |
| Confirmação de que `commit.gpgsign` e `gpg.format = ssh` estão ativos, e que um commit de teste **verifica** (`chaos key verify`) | configuração Git local; relatório do Onboarding |
...
- **`metadata/registries/allowed_signers` é protected path de natureza diferente dos demais.** Os outros governam o comportamento de agentes futuros; este governa **quem conta como humano**. É a raiz de confiança do sistema inteiro (CHAOS §17.6): quem escreve nele concede a si mesmo tudo o que a assinatura protege, de uma vez, para sempre. Três regras próprias: alterá-lo é A4 e exige assinatura humana — recursivamente, pela regra que ele define; linha nunca é apagada, só expirada com `valid-before`, porque apagar torna inverificáveis os commits históricos que aquela chave assinou; e a **primeira** linha é escrita pelo `chaos init`, no Onboarding, que é o único momento em que não há assinatura prévia possível — por necessidade lógica, não por exceção de conveniência, e fora do alcance de qualquer agente porque acontece antes de existir agente.
...
| **verificação de assinatura** (v1.4) | na leitura, a qualquer momento | recusa tratar como humano todo commit sem assinatura verificável (CHAOS §17.6) | não impede a escrita; classifica quem a fez |

Nenhuma é suficiente sozinha, e é por isso que há quatro: a primeira depende do runtime, a segunda do remoto, a terceira do fluxo de PR. Um agente que contorne o hook (runtime diferente) é pego no push; um repositório local-only não tem a segunda e a terceira e depende do `guard` — por isso `default_privacy: local_only` exige `views_builder: local_worker` e um worker com identidade própria. Falha de qualquer camada gera EVT e pausa o RUN.

**A quarta camada é de espécie diferente das três primeiras, e a diferença importa.** As três primeiras são *preventivas* e cada uma depende de um pressuposto que o spike mostrou ser frágil: o `guard` pressupõe que o agente roda no runtime esperado; o `validate` pressupõe remoto; o `CODEOWNERS` pressupõe fluxo de PR. Um agente com shell e credencial de push contorna as três — foi literalmente o que a sessão do spike fez ao comitar como o humano. A verificação de assinatura não previne nada: ela **classifica**, depois do fato, e não pode ser contornada porque não há nada a contornar — o commit ou tem a marca que só a máquina do usuário produz, ou não tem. É a camada que continua valendo quando as outras três falham, e por isso é sobre ela que A4 se apoia.
...
|---|---|---|
| ~~**Spike de identidade** (bloqueante)~~ **CONCLUÍDO em 21/09/2026** | Perguntava se a sessão na nuvem consegue **escolher** a identidade com que comita. **Resposta: consegue.** `git config` e edição de `.git/config` sobrescrevem autor e committer, e o push passa. Resultado completo em CHAOS §0.0. | **Veredito: a premissa caiu, o desenho foi reancorado.** A raiz de confiança saiu do repositório enquanto autoria e voltou como **assinatura**: o executor na nuvem comprovadamente não assina (`commit -S` não gravou assinatura alguma). A4 passa a exigir chave `human:*` (CHAOS §17.6), e o worker local sobe para a Fase 0 como componente de segurança. |
...
| **0 — Fundação e raiz de confiança** | primeiro `chaos-<classe>`; layout; schemas JSON vendorizados em `metadata/schemas/` (§4.5); `AGENTS.md` com as 8 seções de CHAOS §9.4 preenchidas pelo executor do Onboarding a partir das respostas de §3 (a 8ª — backlog — nasce vazia, é do usuário); `order agent sync` gera os bindings, incluindo o Assistente; `.gitattributes`; identidades Git por executor; **autenticação Git verificada; chaves de assinatura geradas; `allowed_signers` criado pelo `chaos init` com a primeira chave humana; `commit.gpgsign` e `gpg.format = ssh` ativos; `chaos key verify` passando**; **esqueleto do worker local, porque é ele que guarda a chave de procedência**; `chaos init/validate/id/commit/sync/key`; `guard` (protected paths incl. enforcement + allowlist); `PAUSED`; teste de alcance dos remotos **e de push direto em `main` a partir da nuvem** | CHAOS AT-03, AT-12, AT-15, AT-18, AT-19, **AT-36, AT-37, AT-38**; ORDER AT-20, AT-21 |
| **1 — Fila e runs** | `chaos <tipo> create/update`; `order run/claim/release`; EVT com union merge; trailers; `validate`; worker completo (claim, A0/A1, checkpoint, **assinatura de procedência**); **contexto mínimo** = bootstrap + state + tarefa, sem busca | ORDER AT-04, AT-07, AT-08, AT-17, AT-18, AT-22; CHAOS AT-07, AT-11, AT-16, AT-20 |
...
| **3 — Equipe** | registry das áreas do onboarding; `order agent sync`; Assistente + áreas + funcionais; delegação; aprovações assíncronas (**A4 e A3 `out_of_band` por commit assinado com chave humana**; pedido redigido pelo Risk Engine, ORDER §18.1) — **depende da Fase 0**: sem chave registrada não há A4 possível, e a fase não tem como sair | ORDER AT-01, AT-05, AT-06, AT-15, AT-19, **AT-30, AT-31, AT-32**; CHAOS AT-17 |
...
**Por que o worker subiu de fase.** Na v1.3 ele entrava na Fase 1, como capacidade de execução — rodar o que a nuvem não roda. Depois do spike ele é a única parte do sistema que vive onde as chaves vivem, e portanto a única que pode produzir a marca sobre a qual A4 repousa. Deixá-lo para a Fase 1 significaria construir Fase 0 e Fase 1 inteiras sobre uma raiz de confiança que ainda não existe — exatamente o erro que a v1.3 cometeu ao deixar o spike para depois de já ter desenhado a garantia que dependia dele.
...
Segredos só em variáveis de ambiente (worker) e no cofre de segredos da plataforma (tarefas agendadas); nunca no repo; `.gitignore` para `.env*`. Tokens de Git por dispositivo.

**Chaves de assinatura (v1.4).** A chave privada humana **MUST NOT** existir em caminho versionado, em nenhuma classe de privacidade, cifrada ou não — um repositório que contenha a chave que o autoriza não autoriza nada. Ela **MUST** ter frase-secreta: sem ela, a assinatura deixa de exigir presença e vira o oráculo que ORDER §18 recusa. A chave do worker vive no diretório do worker, fora do repositório, e é de principal distinto — o worker **MUST NOT** ter acesso à chave humana em nenhuma circunstância, porque um worker que pudesse assinar como o humano seria precisamente a escalação que todo o desenho da v2.6 fecha. `.gitignore` cobre `*.pem`, `id_*`, `*.key` além de `.env*`, e o validador rejeita chave privada em qualquer caminho versionado. Logs sem segredos. Coder só em repositórios de código. Conteúdo externo entra marcado `untrusted`; `guard` bloqueia ação A2+ de um RUN cujo contexto tenha apenas fontes `untrusted` como justificativa. O registry de modelos contém **apenas** fontes declaradas pelo usuário; nenhuma chave de provedor não declarado existe no sistema.
...
7. Aprovação A3 induzida por injeção de prompt — vetor fechado pelo valor inicial `a3_mode: out_of_band`, que na v1.4 significa **commit assinado por chave humana** e não mais "commit sem trailer". O fechamento ficou mais forte: antes dependia de o modelo não conseguir produzir um commit sem trailer, o que o spike mostrou ser trivial; agora depende de ele não conseguir produzir uma assinatura, o que o spike mostrou ser impossível no sandbox. Só reaberto por decisão explícita na Fase 4 (§13.1), com regressão automática se as métricas piorarem.
...
11. **A máquina do usuário virou alvo único (v1.4).** Concentrar a raiz de confiança numa máquina é a decisão certa e tem o custo óbvio: quem comprometer a máquina e a frase-secreta assina o que quiser, e não há segunda barreira. A v2.6 compra redução de superfície — de "qualquer sessão na nuvem, com dois comandos" para "acesso físico ou remoto à máquina **e** à frase-secreta" — não invulnerabilidade, e descrever isso como prova absoluta repetiria o erro que o spike corrigiu. Mitigações proporcionais, em ordem de custo: frase-secreta forte e agente SSH com expiração curta; a médio prazo, chave humana em hardware (FIDO2/`ed25519-sk`), que troca "a frase-secreta vazou" por "o dispositivo físico foi roubado"; e, em qualquer caso, `valid-before` numa rotação periódica, que limita a janela de uma chave comprometida sem invalidar o histórico.
12. **Perda da chave humana é perda de autonomia, não de dados (v1.4).** Se a máquina morrer com a única chave humana, nada se perde do conteúdo — entidades são Markdown em Git — mas nenhuma A4 volta a ser possível até que uma nova chave seja registrada, e registrar exige assinatura da chave que já não existe. Mitigação obrigatória no Onboarding: **duas** chaves humanas desde o início, em máquinas ou mídias diferentes, ambas no `allowed_signers`. Não é redundância opcional; é a diferença entre reconfigurar e reconstruir o repositório do zero.
...
**A montagem é um script, não um roteiro (v1.4).** `toolkit/montar-pacote.ps1` e `toolkit/montar-pacote.sh` montam o pacote inteiro a partir de um repositório já construído: baixam o uv e o Python gerenciado para dentro da pasta, criam o venv, instalam as dependências, semeiam `tools-seed/`, baixam o PortableGit no Windows, **verificam que nenhum segredo entrou** e escrevem o `TOOLKIT.yaml` com os hashes do que de fato foi incluído. Um manifesto escrito à mão afirma o que ninguém conferiu.

Três invariantes que o script **verifica e que abortam a montagem**, cada uma correspondendo a um modo de falha que produz um pacote aparentemente completo:

1. **`uv python install --managed-python`.** Sem `--managed-python`, numa máquina de montagem que já tenha Python 3.13, o uv usa o do sistema: `python/` fica vazia e o venv aponta para um interpretador que não existe no destino. O script confere que a pasta não ficou vazia.
2. **`uv venv --relocatable --seed`.** Um venv comum grava caminhos absolutos e quebra ao mudar de máquina ou de letra de unidade, quase sempre com um erro que não diz isso. O script confere que o `home` do `pyvenv.cfg` aponta para dentro do pacote. `--seed` existe porque o uv **não tem** subcomando de download (a redação anterior desta seção pedia `uv pip download`, que não existe) — quem baixa as rodas é o pip do próprio venv.
3. **Nenhum segredo.** Cabeçalho de chave privada em qualquer arquivo, `*.pem`, `*.key`, `.env*` ou um `allowed_signers` com linha preenchida abortam a montagem. É a única verificação que recusa o pacote em vez de avisar, e é assim de propósito (§18.3).
...
Isto não é cuidado genérico, é estrutural, e a v1.4 tornou-o mais grave. Toda a garantia A4 repousa agora sobre **assinatura por chave que existe numa máquina só** (CHAOS §17.6). Uma chave de assinatura que viajasse no pacote estaria, por construção, em toda máquina que o baixou — e a marca deixaria de significar "esta pessoa, nesta máquina" para significar "quem quer que tenha descompactado o ZIP". Seria pior do que não ter assinatura, porque teria a aparência de garantia.

Acrescenta-se portanto à lista, explicitamente: **nenhuma chave privada de assinatura, e nenhum `allowed_signers` preenchido.** O pacote leva o arquivo como *modelo vazio com o formato e um comentário explicando que a primeira linha nasce no `chaos init`*. O pacote leva ferramentas e **modelos** de configuração; as credenciais e as chaves nascem por máquina, no Onboarding (§3.1).
