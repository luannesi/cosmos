<#
    ...
    LIMITE DELIBERADO: este assistente não age fora da pasta do pacote e do
    repositório que você indicar. Ele não instala o Claude Code, não registra o
    worker no agendador e não escreve credencial nenhuma — imprime os comandos
    no fim para você executar. A promessa "nada foi instalado fora desta pasta"
    é verificável, e um assistente que a quebrasse por conveniência tornaria o
    pacote impossível de auditar.
#>
[CmdletBinding()]
...
Write-Host "      (primeira máquina; abre o Onboarding, que faz as perguntas de verdade)"
Write-Host ""
Write-Host "  [3] Só preparar o ambiente, sem repositório"
Write-Host ""
...
if ($escolha -eq '2') {
    ...
    $seed = Join-Path $Root 'tools-seed\chaos\chaos.py'
    ...
    Write-Host "     python tools\chaos\chaos.py onboarding run"
    Concluir $destino
    exit 0
}
...
Write-Host ""
Write-Host "     .\INICIAR.cmd        (e escolha 1 ou 2)"
Write-Host ""
