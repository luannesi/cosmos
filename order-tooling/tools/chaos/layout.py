...
O sistema do qual este repositório faz parte chama-se COSMOS: as duas camadas
-- CHAOS e ORDER -- mais o binding que as realiza nesta máquina.

CHAOS = Capture -> Harvest -> Atomize -> Organize -> Synthesize. Os cinco
estágios são o pipeline que este repositório instancia: material bruto entra
sem julgamento (`inbox/`, camada episódica), vira conhecimento por um ato
explícito (`chaos promote`), cada coisa é uma unidade endereçável com ID
estável, ganha lugar e relação e vigência, e o conjunto responde a perguntas
que nenhuma parte responde sozinha (`chaos context`, `chaos search`).

Governança — risco, permissão, aprovação, cota — não está nos cinco verbos
porque não é daqui: é do ORDER (Orchestrated Runtime for Distributed Execution
& Reasoning), que opera este repositório sem ser sua fonte de verdade e
persiste o próprio estado em `order/`.
