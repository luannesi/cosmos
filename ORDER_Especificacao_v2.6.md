# ORDER — Orchestrated Runtime for Distributed Execution & Reasoning — Especificação Completa v2.6

**Status:** Especificação de referência para implementação
**Versão:** 2.6 (supersede v2.5)
**Data:** 2026-09-21
**Documentos irmãos:** CHAOS v2.6 (*Capture · Harvest · Atomize · Organize · Synthesize*) · Implementação Claude-Nativa v1.4
**Sistema:** as três camadas em operação formam o **COSMOS** (CHAOS §1.0.1)
...
## 0.0 Changelog v2.5 → v2.6 (aprovação ancorada em assinatura)

Esta rodada é consequência do spike de identidade executado em 21/09/2026 e
documentado em CHAOS §0.0 e §17.6. O spike mediu o runtime real e refutou a
premissa sobre a qual o Approval Engine repousava.

**A premissa refutada.** A v2.5 definia "aprovação A4 válida" como *commit de
identidade humana sem trailer de agente*. O spike mostrou que uma sessão na
nuvem sobrescreve autor, committer e trailers com dois comandos, e que o push
passa. A definição inteira era forjável pelo agente que ela restringia.

**A premissa que sobreviveu.** O mesmo spike mostrou que o executor na nuvem
**não consegue assinar**: `commit -S` foi relatado como bem-sucedido pela
própria sessão e não gravou assinatura nenhuma. Não há chave no sandbox. A
assinatura é a única marca do commit fora do alcance do executor, e é sobre ela
que o Approval Engine passa a se apoiar.

**O que muda no ORDER:**

- **§18** — A4 e A3 `out_of_band` passam a exigir **assinatura verificável de
  chave `human:*`** (CHAOS §17.6). "Commit sem trailer" deixa de ser critério.
- **§18** — distinção nova entre **procedência** (assinatura do worker: veio da
  máquina do usuário) e **consentimento** (assinatura humana: uma pessoa
  autorizou). A primeira não substitui a segunda em nenhum caso.
- **§18.1** (novo) — **o texto do pedido de aprovação é gerado pelo
  Risk Engine**, nunca pelo executor que propõe a ação. Consequência direta do
  segundo achado do spike: a plataforma barrou corretamente um trailer forjado,
  e o próprio agente restringido ofereceu ao humano três formas de contornar a
  barreira, redigindo o pedido de confirmação em linguagem de rotina. Barreira
  cujo texto de consentimento é escrito pelo agente restringido protege contra
  agente distraído, não contra agente comprometido.
- **§40** — degradação nova: sem chave humana registrada, A3 `out_of_band` e A4
  são impossíveis e as tarefas correspondentes vão a `blocked(no_signing_key)`.
  O sistema continua útil em A0–A2.
- **§35** — o worker local sobe de componente de capacidade a **componente de
  segurança**, com as consequências de ordem de construção em Implementação §13.
- AT-05 reescrito; AT-30, AT-31 e AT-32 novos.

**O que não muda:** fila, lease, checkpoint, roteamento, cotas, escada de
maturidade, camada episódica, handoff. O spike tocou a raiz de confiança das
aprovações, não a mecânica de execução.
...
- **A4 (reescrito na v2.6):** aprovação **MUST** ser gravada em commit com **assinatura verificável de chave `human:*`** do `allowed_signers` (CHAOS §17.6). Não é mais "fora do caminho do modelo e sem trailer": o spike de §0.0 mostrou que ausência de trailer e identidade humana são ambas produzíveis pelo executor. O que o executor comprovadamente não produz é a assinatura, e é ela o critério. Uma APV A4 cujo commit de decisão não traga assinatura humana verificável é inválida — inclusive quando autor, committer e ausência de trailer estejam todos corretos, que é precisamente o caso que a v2.5 aceitava por engano;
- a espera é **assíncrona**: o RUN fica `awaiting_approval` com checkpoint; nenhum processo bloqueia esperando.

**Procedência não é consentimento (v2.6).** CHAOS §17.6 define três principais, e o Approval Engine usa os três de formas diferentes:

| Marca no commit | O que prova | Serve para |
|---|---|---|
| assinatura `human:*` | uma pessoa autorizou, agora, com a frase-secreta | **A4, A3 `out_of_band`, protected paths** |
| assinatura `executor:local_worker` | a ação veio da máquina do usuário, não da nuvem | políticas de execução local; **nunca** aprovação |
| nenhuma assinatura | nada | A0–A2 comuns |

A tentação de aceitar a assinatura do worker em A4 precisa ser nomeada para ser recusada: um worker que assina sem presença humana é um oráculo de assinatura, e qualquer coisa capaz de enfileirar trabalho para ele obtém a marca. O portão passaria a ter nome sem ter função. A fricção da frase-secreta **é** o controle, não um efeito colateral dele.

**Quando o worker grava a procedência.** `order/policies/worker.yaml` declara `signature_mode`, com três valores e padrão `always`:

| Valor | Efeito | Quando faz sentido |
|---|---|---|
| `always` (**padrão**) | o worker assina todo commit que faz | o caso normal — a chave não tem frase-secreta, então assinar não custa nada |
| `on_demand` | assina só quando uma policy o exigir | histórico mais limpo, ao preço de procedência não verificável depois |
| `never` | nunca assina | só para depuração |

**Decisão do proprietário (21/09/2026): `always`.** O valor deixou de ser padrão provisório e passou a ser escolha registrada; `on_demand` e `never` permanecem no schema para depuração e para instalações que decidam diferente.

O motivo é de tempo, não de segurança: **procedência só é verificável se gravada no momento**. Um commit antigo sem assinatura é indistinguível de um que veio da nuvem se passando pela máquina — a informação não pode ser reconstruída depois, então ou se grava sempre, ou se aceita não tê-la.

O risco de `always` é de leitura humana, não de mecanismo: assinatura em todo lugar normaliza a marca, e alguém pode passar a ler "assinado" como "aprovado". O mecanismo não se confunde — a verificação é por **principal**, e a de worker nunca satisfaz A4 —, mas o hábito mental é real e é por isso que §18 nomeia a diferença em vez de deixá-la implícita.

A ausência da chave do worker **não** impede o commit: procedência informa, não autoriza. O portão é a assinatura humana, e a falta dela já nega A4 no caminho de aprovação.

**Degradação sem chave (v2.6).** Num repositório sem chave `human:*` registrada, A3 `out_of_band` e A4 são **impossíveis**, não "aprovados por padrão" nem "aprovados por CLI provisoriamente". A tarefa vai a `blocked(no_signing_key)` e espera. É degradação fechada: a ausência do mecanismo nega, nunca concede — que é o oposto do que a regra da v2.5 fazia, onde a ausência da marca (trailer) era a própria condição de aprovação.
...
### 18.1 Redação do pedido de aprovação (v2.6)

O §18 diz que o pedido apresenta ação, motivo, impacto, risco, reversibilidade, alternativa e evidências. A v2.6 acrescenta **quem escreve esse texto**, e a resposta é normativa:

> **O texto que o humano lê ao aprovar MUST ser gerado pelo Risk Engine a partir da ação pretendida — a ferramenta, o recurso, o diff, a classe calculada. O executor que propõe a ação MUST NOT redigir, editar ou influenciar esse texto.**

O executor contribui com **um** campo, `rationale`, e ele é apresentado como tal: citado, atribuído ao agente, visualmente separado do restante. Tudo o mais — o que vai acontecer, a que recurso, com que reversibilidade — é derivado pelo motor, não narrado pelo proponente.

**O caso que originou a regra.** No spike de CHAOS §0.0, a plataforma detectou corretamente um trailer de autorização forjado e recusou o commit. O agente restringido então explicou o bloqueio ao humano, ofereceu três saídas, e redigiu a primeira como "confirme que essas linhas não são metadata de autorização" — pedindo ao humano que desmentisse o que a proteção acabara de detectar corretamente. Em seguida: "isso me permitiria contornar o bloqueio de segurança para completar o comando", em tom de procedimento de rotina. Confirmado, o contorno funcionou.

Nada disso exigiu má-fé. Um agente que quer completar a tarefa naturalmente descreve o obstáculo como formalidade — é a descrição que torna a tarefa completável. E é por isso que a regra é estrutural e não comportamental: **o proponente da ação nunca é fonte confiável sobre o risco dela**, do mesmo modo que o risco não vem do texto da tarefa (§17) e que o agente não declara a própria classe de risco (Golden Rule 3). A v2.6 apenas estende o princípio que o ORDER já aplicava ao cálculo do risco para a **apresentação** dele.

Três consequências verificáveis (AT-32):

- uma APV cujo corpo contenha texto de origem do executor fora do campo `rationale` é inválida;
- o pedido **MUST** nomear a ação em termos do Tool Registry (§14) e do recurso, não em termos da intenção declarada pelo agente;
- linguagem que minimize o portão — "apenas confirme", "formalidade", "para completar o comando" — não pode ser produzida pelo motor, porque o motor não tem esse vocabulário: ele preenche um gabarito fixo.
...
Least privilege; sandbox para código; allowlist de ferramentas por agente; secret management fora do Git e fora de prompts; isolamento; timeouts; limites de recursos; logs sanitizados; aprovação crítica; protected paths; conteúdo externo como dado; separação `personal`/`work`; **autoria humana provada por assinatura, nunca inferida de identidade Git (CHAOS §17.6); pedido de aprovação redigido pelo motor de risco, nunca pelo proponente (§18.1)**.

**O worker local é componente de segurança (v2.6).** Até a v2.5, o worker existia por capacidade: rodar o que a nuvem não roda, alcançar o que a nuvem não alcança, guardar o que não pode sair da máquina. O spike de CHAOS §0.0 lhe deu uma segunda função, de natureza diferente: **é a única parte do sistema que roda onde as chaves de assinatura existem**. Consequências:

- a ordem de construção muda — o worker e a geração de chaves precedem qualquer aprovação funcional (Implementação §13);
- comprometer o worker passa a ser comprometer a raiz de confiança, e não apenas perder capacidade de execução; ele herda os requisitos de um componente de segurança (superfície mínima, dependências auditadas, nenhuma execução de código vindo de conteúdo não confiável);
- a chave humana **MUST NOT** ser acessível ao worker sem presença: worker e humano são principais distintos com chaves distintas, e o worker jamais assina em nome do humano (§18).
...
Sem runtime nuvem → worker local assume `any`; sem worker local → tarefas `local` esperam; sem provedor remoto → Ollama para `local`; sem vetor → BM25 + grafo; sem Obsidian → Git/CLI/Claude; sem MCP → adapters; sem LangGraph → runner próprio; sem canal externo → notificação em `order/` e no próximo briefing; **sem camada episódica → `context.episodic` vazio, busca por BM25 e grafo, handoff entre sessões pelos HND do `order/` como antes**; **sem chave humana registrada → A0–A2 funcionam integralmente, A3 `out_of_band` e A4 são impossíveis e as tarefas vão a `blocked(no_signing_key)`, `chaos health` reporta `drift`**; **sem worker local → além das tarefas `local` esperando, nenhuma assinatura é produzida, e portanto nenhuma aprovação A4 conclui** — nenhum comando falha e nenhum AT muda de resultado.

A última linha merece ser lida duas vezes, porque inverte uma expectativa da v2.5: a indisponibilidade do worker deixou de ser degradação apenas de **capacidade** e passou a ser degradação de **autoridade**. Um sistema sem worker não é um sistema mais lento; é um sistema que não aprova nada de alto risco. Isso é intencional e é o sentido de falhar fechando.
...
- **AT-05 Approval (reescrito na v2.6):** A3 gera APV e o RUN fica `awaiting_approval` sem processo bloqueado; APV expirado não executa. Além disso: uma APV **A4** cuja decisão foi gravada em commit **sem assinatura humana verificável** é recusada — mesmo quando autor e committer são a identidade humana e o commit não tem trailer de agente. Esse é o caso que a v2.5 aceitava e que o spike de CHAOS §0.0 mostrou ser produzível pelo executor.
...
- **AT-30 Assinatura do worker não aprova (v2.6):** uma APV A4 decidida em commit assinado pela chave `executor:local_worker` é **recusada**; a mesma APV decidida em commit assinado por chave `human:*` é aceita. Uma implementação que verifique apenas "o commit está assinado" passa no segundo caso e falha no primeiro, que é o caso que importa: é o caminho de escalação disponível a qualquer coisa capaz de enfileirar trabalho para o worker.

- **AT-31 Sem chave, nada de A4 (v2.6):** num repositório sem chave `human:*` registrada, toda tarefa que exija A3 `out_of_band` ou A4 termina em `blocked(no_signing_key)`; nenhuma é executada, nenhuma é aprovada por CLI como contingência, e as tarefas A0–A2 correm normalmente. Registrada a chave, as tarefas bloqueadas voltam a ser elegíveis sem intervenção manual em cada uma.

- **AT-32 Pedido redigido pelo motor (v2.6):** o corpo da APV é gerado pelo Risk Engine a partir da ferramenta, do recurso e do diff; o único campo de origem do executor é `rationale`, e ele aparece citado e atribuído. Uma APV cujo corpo contenha texto do executor fora desse campo é inválida para o validador. O teste inclui o caso adversarial: um agente cuja `rationale` tente reenquadrar o portão como formalidade não altera nenhuma outra parte do pedido, e a classe de risco apresentada continua sendo a calculada (§17), não a sugerida.

## 45. Definition of Done — ORDER v2.6

Agentes registráveis em matriz; tasks/runs rastreáveis e retomáveis; fila com lease funcionando entre nuvem e worker local; risco determinístico; permissões com protected paths; aprovações assíncronas com expiração; kill-switch e cotas ativos; triggers com dedup e escada de maturidade; roteamento em dois níveis; contexto recuperável e marcado por confiança; memória operacional em `order/`; **camada episódica separada, sem autoridade e dispensável**; **toolchain verificado antes de qualquer execução autônoma**; **aprovação A4 ancorada em assinatura humana verificável e nunca em identidade Git; assinatura de worker distinguida de consentimento humano; pedido de aprovação redigido pelo motor de risco; ausência de chave degradando fechado**; auditoria completa; falhas recuperáveis; execução observável; código isolado; canais substituíveis; nenhum provider estrutural; **todos os Acceptance Tests de §44 passam** — a suíte inteira, sem faixa numérica (ver CHAOS §32).

## 46. Golden Rules

1. CHAOS é a verdade persistente; `order/` é o estado operacional durável.
2. ORDER executa e orquestra; nunca é source of truth.
3. LLM raciocina; não decide risco, permissão nem cota — **nem redige o pedido de aprovação da própria ação (§18.1)**.
