# CHAOS — Capture · Harvest · Atomize · Organize · Synthesize — Especificação Completa v2.6

**Status:** Especificação de referência para implementação
**Versão:** 2.6 (supersede v2.5)
**Data:** 2026-09-21
**Idioma normativo:** PT-BR
**Documentos irmãos:** ORDER v2.6 · Implementação Claude-Nativa v1.4
...
## 0.0 Changelog v2.5 → v2.6 (âncora de identidade por assinatura)

Esta rodada não nasceu de análise: nasceu de **medição**. O spike de identidade
previsto no §17.1 da v2.5 foi executado em 21/09/2026 contra o runtime real, e
refutou a pré-condição sobre a qual repousava a garantia A4.

**O que foi medido.** Seis commits feitos por uma sessão Claude Code na nuvem
sobre um repositório descartável, lidos depois no histórico, na máquina do
usuário:

| Caminho | Autor | Committer | Assinatura |
|---|---|---|---|
...
| `commit -S` | escolhido | escolhido | **ausente — o `-S` não produziu assinatura** |

**As quatro conclusões:**

1. **Existe identidade nativa distinta.** Sem interferência, o executor da nuvem
   comita com identidade própria em autor e committer. A separação existe.
2. **Mas identidade não é controle.** `git config` e edição de `.git/config`
   sobrescrevem **autor e committer**, e o push passa. Nenhum campo textual do
   commit é evidência de quem o fez.
3. **`--author` e `GIT_AUTHOR_*` alteram só o autor**, preservando o committer.
   É uma distinção real e inútil como garantia: quem forja usa o caminho que
   funciona, não o que falha.
4. **O executor na nuvem não consegue assinar.** O `-S` foi relatado como
   bem-sucedido pela própria sessão e **não gravou assinatura nenhuma** no
   histórico. Não há chave utilizável no sandbox. Esta é a única dimensão do
   commit fora do alcance do executor — e o auto-relato divergente é, ele
   próprio, evidência de que auto-relato de executor não é fonte.

**O que muda.** A regra "commit humano = identidade humana **e** ausência de
trailer" está morta: as duas metades são forjáveis. Ela é substituída pela
forma positiva, que a conclusão 4 sustenta:

> **Um commit conta como humano quando traz assinatura verificável contra chave
> registrada como humana. Sem assinatura verificável, é executor — quaisquer
> que sejam autor, committer e trailers.**

A inversão de polaridade é o ponto. A regra antiga inferia autoria humana pela
**ausência** de uma marca, e ausência é o que qualquer um produz de graça. A
nova exige a **presença** de uma marca que o executor comprovadamente não
produz.

**Consequências normativas:**

- §17.1 reescrito: `Actor:` derivado da **assinatura**, não da credencial;
  `executors.yaml` rebaixado de mecanismo de autorização a registro de
  atribuição e quota.
- §17.6 novo: contrato de assinatura — formato SSH, `allowed_signers`
  vendorizado como protected path, o que o validador verifica.
- §17.4: assinatura sai de "extensão fora do MVP" e entra no núcleo.
- §20: novas verificações de assinatura.
- §21: `chaos key` (gerar, registrar, verificar).
- AT-19 e AT-27 reescritos; AT-36, AT-37 e AT-38 novos.
- §31: novo anti-padrão — barreira de segurança cujo texto de confirmação é
  redigido pelo agente que ela restringe.

**Acréscimo fora do escopo do spike:** §1.0 nova — o significado de **CHAOS**
(*Capture · Harvest · Atomize · Organize · Synthesize*), que atravessou quinze
rodadas sem nunca ter sido escrito. Não muda contrato nenhum; muda quanto do
desenho se lê a partir do nome.

**O que sobrevive intacto:** toda a camada de conteúdo — entidades, PMMS,
bitemporalidade, camada episódica, proveniência, views, retrieval. O spike
tocou a raiz de confiança, não o modelo de dados.

---

## 0.0 Changelog v2.4 → v2.5 (contrato de `chaos health`)

| # | Mudança | Motivo |
|---|---|---|
| 1 | §21.1 nova: **contrato de `chaos health`** — o comando estava na CLI desde a 10ª rodada sem semântica alguma. Agora verifica que o toolchain **em execução** corresponde ao **vendorizado**, e distingue três estados (`ok`, `drift`, `broken`) | Comando declarado sem contrato é comando que cada implementação inventa. Pior: §18 manda vendorizar `tools/` por tag e nada verificava que o binário que roda é o dessa tag — um repositório atualizado operado por ferramenta antiga produz defeito que ninguém explica, e ORDER §39 promete reprodutibilidade que não tinha como ser conferida |
| 2 | `metadata/tooling.yaml` declara a tag vendorizada e entra nos protected paths de §4.1 | Pelo critério — todo arquivo que governa o comportamento de agentes futuros — o arquivo que decide **qual ferramenta executa** é governança, não conteúdo. Deixá-lo livre permitiria a um agente apontar o sistema para outra versão das próprias ferramentas com um commit A1 |
| 3 | AT-35 novo | Regra normativa sem teste |
| 4 | §17.1: **a ordem do ledger é da leitura**, não do disco — `merge=union` não ordena e não tem como | Descoberto ao implementar: a AT-16 exigia ordem física depois de um merge de união, que é impossível por construção. O invariante real é ausência de perda mais `ts` como chave total |
| 6 | §21: `chaos area` e as três consultas de grafo (`neighbors`, `path`, `subtree`) declaradas | §9 exige Área State e §15 nomeia as três consultas; nenhuma tinha comando. O grafo era gravado e nunca lido — índice que ninguém consulta é peso morto |
| 5 | §20: a exigência de EVT por commit vale para **executores**, não para o humano | AT-01 e AT-07 se contradiziam ponto a ponto. Implementação §11 já resolvia: edição manual por humano é aceita e reconciliada; escrita de entidade por executor passa pelo CLI |
...
**CHAOS** = **C**apture → **H**arvest → **A**tomize → **O**rganize → **S**ynthesize.

Não é rótulo: é o pipeline que a especificação inteira instancia, e cada estágio
tem maquinário próprio e verificável aqui dentro.

| Estágio | O que significa | Onde vive |
|---|---|---|
| **Capture** | material bruto entra sem ser julgado — sessão, recorte, anotação, arquivo externo | `inbox/` com `origin: external_source` (§14.1); camada episódica (§4.2) |
| **Harvest** | do bruto se extrai o que merece virar conhecimento — e essa passagem é explícita, nunca automática | `chaos promote` (§21), único caminho episódico → canônico |
| **Atomize** | cada coisa vira uma unidade endereçável, com identidade estável e independente das demais | um arquivo por entidade, IDs estáveis (§7.2), Markdown + frontmatter (§7.1) |
| **Organize** | as unidades ganham lugar, relação, vigência e autoridade | PARA e áreas (§5), relações tipadas (§7.6), bitemporalidade e `authority` (§6, §7.1), grafo (§19) |
| **Synthesize** | o conjunto responde a uma pergunta — o que se monta a partir das partes, não a soma delas | retrieval (§15), Context Builder (§16), views derivadas (§19) |

Lido assim, o nome é um teste de coerência: **um mecanismo desta especificação
que não sirva a nenhum dos cinco estágios está no documento errado.** Foi o que
aconteceu com a camada episódica até a v2.4, tratada como fonte quando é
Capture; e o que o `chaos promote` corrige ao tornar o Harvest um ato, e não um
efeito colateral.

### 1.0.1 COSMOS — o nome do conjunto

CHAOS é uma camada; ORDER é outra; o binding é a terceira. **O sistema que
resulta dos três rodando juntos chama-se COSMOS.**

Não é acrônimo, e não precisa ser: *kósmos* é a palavra grega para o mundo
ordenado que emerge do *kháos*. O par já estava no vocabulário do projeto antes
de alguém notar — CHAOS é de onde se parte, ORDER é o que ordena, COSMOS é o
que resulta.

A escolha é deliberada num ponto: **o binding não recebe nome próprio.** Ele é
o membro substituível do trio — o Perfil B (§27.3) existe para ser outro
binding com os mesmos contratos —, e batizá-lo sugeriria uma permanência que
ele não deve ter. Nomear o conjunto, e não a implementação corrente, mantém o
nome válido quando a infraestrutura mudar.

Uso: "CHAOS" e "ORDER" nomeiam as camadas e continuam sendo os termos
normativos deste e do documento irmão; "COSMOS" nomeia o sistema instalado e
em operação — é o termo do tutorial, do `AGENTS.md` e da conversa cotidiana,
não das cláusulas.

**O que o nome não cobre, e é preciso dizer.** Governança, risco, permissão,
aprovação, cota — nada disso está nos cinco verbos, porque nada disso é do
CHAOS: é do ORDER (*Orchestrated Runtime for Distributed Execution &
Reasoning*), que opera esta camada sem ser sua fonte de verdade. Integridade e
proveniência (§14, §17) também ficam de fora do acrônimo, e são do CHAOS — são
a condição para que os cinco estágios signifiquem algo, não um sexto estágio.
Um nome que abarcasse tudo seria mais completo e diria menos.

**Registro honesto:** esta seção nasceu na v2.6, depois de quinze rodadas — duas
delas com revisores adversariais cruzados — em que o termo foi usado centenas de
vezes sem nunca ser definido. Ninguém perguntou o que significava porque todos
já o usavam com naturalidade. É a mesma classe de defeito do `chaos health`
declarado sem contrato por quatro rodadas: **termo familiar para de ser
examinado.**
...
| `metadata/registries/allowed_signers` | **decide quem é humano** — é a raiz de confiança do sistema (§17.6); quem escreve aqui concede a si mesmo tudo |
...
| `metadata/tooling.yaml` | **declara a tag das ferramentas vendorizadas — decide qual código executa tudo o mais** |
...
**Autoria:** todo commit feito por agente **MUST** carregar os trailers `Actor: agent:<id>` e `Surface: <superfície>`. Cada executor **SHOULD** ter identidade Git própria (conta de serviço, deploy key ou app), distinta da identidade do humano.

**Os trailers e a identidade Git são atribuição, não autorização.** Esta distinção é o resultado do spike de 21/09/2026 (§0.0) e é a correção mais importante da v2.6. Autor, committer e trailers são texto que o executor escreve; um executor na nuvem sobrescreve os três com dois comandos, e o push passa. Servem para saber **quem diz ter feito** — útil para atribuição, quota e depuração. **MUST NOT** ser usados como prova de quem fez.

**A prova é a assinatura.** A regra normativa de autoria é esta, e substitui integralmente a regra de ausência de trailer da v2.5:

> **Um commit conta como humano quando, e somente quando, traz assinatura
> verificável contra uma chave registrada como humana em
> `metadata/registries/allowed_signers` (§17.6). Todo commit sem assinatura
> verificável conta como executor, independentemente de autor, committer e
> trailers.**

Três propriedades desta forma, que a anterior não tinha:

- **É positiva.** Infere autoria humana pela presença de uma marca, não pela ausência dela. Ausência é o estado natural de qualquer commit e custa zero para produzir; a assinatura exige a chave privada.
- **Falha fechada.** Commit sem assinatura é classificado como executor, que é a classe de **menor** privilégio. Uma falha do mecanismo de assinatura nega aprovação; não a concede. Na regra antiga, a falha equivalente — trailer ausente — concedia.
- **É verificável offline.** `git log --show-signature` contra o `allowed_signers` vendorizado; não depende do hosting, de API, nem de o executor cooperar.

**O que `executors.yaml` faz agora.** Continua protected path e continua mapeando `credencial → ator`, mas para **atribuição**: preencher o campo `actor` do EVT, contabilizar quota (ORDER §21), e identificar de qual executor veio um RUN. Um ator ausente do registry continua fazendo `chaos commit` recusar — é higiene, não fronteira. O que **MUST NOT** mais acontecer é qualquer decisão de autorização consultar esse arquivo: autorização consulta assinatura.

**O que o validador verifica agora** (§20): todo commit que toca protected path **MUST** ser assinado por chave humana; toda APV A4 **MUST** ser decidida em commit assinado por chave humana; um commit assinado cuja assinatura não verifica contra o `allowed_signers` é violação `integrity` — e é a única violação desta seção que não tem correção automática possível, porque significa ou chave revogada, ou histórico adulterado.

**O que continua sendo verdade da v2.5:** commit de identidade de executor com trailer ausente permanece anomalia registrada como warning — não porque prove algo, mas porque um executor bem-comportado sempre põe o trailer, e a ausência indica defeito de implementação ou caminho de escrita não governado. Warning, não `integrity`: a v2.5 tratava isto como fronteira e era o erro.
...
Hash chain (`previous_event_hash`), Merkle root periódico e âncora externa são extensões da Fase 6; blockchain é somente âncora opcional, nunca banco, memória ou source of truth.

**Assinatura deixou de ser extensão.** Na v2.5 a assinatura figurava aqui, como refinamento adiável da Fase 6. O spike de §0.0 a promoveu ao núcleo: ela é a raiz de confiança do sistema, não um reforço opcional dele. O contrato está em §17.6 e vale desde o MVP. O que permanece na Fase 6 é a **cadeia** de hashes do ledger, que é problema diferente — integridade da sequência de eventos, não autoria de commits.
...
**Ordem do ledger é da leitura, não do disco.** `merge=union` concatena os dois lados; ele não ordena, e não tem como — o driver não conhece a semântica das linhas. Exigir ordem física de um log append-only com dois escritores concorrentes é exigir o impossível. O que o formato garante é que **nenhuma linha se perde** e que `ts` é chave de ordenação total; quem precisa de cronologia ordena ao ler. `chaos audit verify` confere integridade (sem `event_id` duplicado), não ordem física.

### 17.6 Contrato de assinatura (v2.6)

**Formato: assinatura SSH, não GPG.** Git suporta `gpg.format = ssh` desde a 2.34. A escolha é deliberada e tem três razões: dispensa instalar GnuPG em cada máquina, o que num parque heterogêneo é a diferença entre um passo e uma tarde; reaproveita a chave que o usuário já precisa ter para falar com o remoto; e a lista de quem pode assinar é um **arquivo de texto versionado no repositório**, o que a torna, ela própria, protected path sob a mesma regra de todos os demais — com GPG, a lista vive num chaveiro fora do repositório, invisível ao validador.

```ini
[gpg]
    format = ssh
[gpg "ssh"]
    allowedSignersFile = metadata/registries/allowed_signers
[commit]
    gpgsign = true
```

**Três principais, não dois.** A v2.5 conhecia humano e executor. A v2.6 precisa de três, porque o spike mostrou que "não é a nuvem" e "é o humano" são afirmações diferentes:

| Principal | Chave | Onde vive | Exige presença | O que prova |
|---|---|---|---|---|
| `human:*` | chave humana | máquina do usuário, protegida por frase-secreta ou hardware | **sim** | uma pessoa autorizou esta ação agora |
| `executor:local_worker` | chave do worker | máquina do usuário, sem frase-secreta | não | esta ação veio da máquina do usuário, não da nuvem |
| `cloud:*` | nenhuma | — | — | nada; toda ação da nuvem é não assinada por construção |

**Por que a chave do worker não basta para A4.** Um worker que assina sem presença humana é um oráculo de assinatura: qualquer coisa que consiga enfileirar trabalho para ele obtém a marca. Se A4 aceitasse a assinatura do worker, a escalação seria "peça ao worker" e o portão teria nome, não função. Portanto:

- **A4 e escrita em protected path** exigem assinatura de chave `human:*`. Sem frase-secreta digitada — ou toque em chave de hardware — não há assinatura, e é exatamente essa fricção que constitui o controle.
- **A3 sob `a3_mode: out_of_band`** exige assinatura `human:*` pela mesma razão.
- **A assinatura do worker** distingue execução local de execução na nuvem, e é o que habilita políticas do tipo "esta classe de tarefa só roda na máquina do usuário". É garantia de **procedência**, não de **consentimento**.

**O arquivo `allowed_signers`.** Vive em `metadata/registries/allowed_signers`, é protected path por §4.1 (`metadata/registries/**`), e segue o formato do OpenSSH:

```text
luan@exemplo.com namespaces="git",valid-after="20260921" ssh-ed25519 AAAAC3Nza... humano maquina-principal
worker@exemplo.com namespaces="git",valid-after="20260921" ssh-ed25519 AAAAC3Nza... worker maquina-principal
```

> **As opções são separadas por vírgula, num único campo — nunca por espaço.** A primeira redação desta seção usava espaço, e a implementação encontrou o defeito: o OpenSSH lê o segundo campo como início da chave e rejeita a linha inteira com `invalid key`. O efeito é pior do que um erro de sintaxe, porque `git log --format=%G?` devolve `U` em vez de `G` — uma assinatura boa que não verifica — e o sistema nega uma aprovação legítima sem dizer por quê. O comentário no fim da linha também não aceita vírgula, pelo mesmo motivo.

Quatro regras normativas sobre ele:

1. **A chave privada nunca entra no repositório, em nenhuma classe de privacidade, nem cifrada.** Um repositório que contenha a chave que o autoriza não autoriza nada. O pacote portátil (Implementação §18) **MUST NOT** conter chave alguma; as chaves nascem no Onboarding, por máquina.
2. **Linha nunca é removida — é expirada.** Revogar uma chave é acrescentar `valid-before` à linha existente, jamais apagá-la. Apagar torna inverificáveis todos os commits históricos que ela assinou, e um histórico que deixa de verificar retroativamente é indistinguível de um histórico adulterado.
3. **O papel é declarado pelo principal, não por comentário.** O identificador à esquerda determina se a chave é humana ou de worker, via `metadata/registries/executors.yaml`. Comentário no fim da linha é para gente, não para código.
4. **Alterar o arquivo é A4 e exige assinatura humana** — recursivamente, pela regra que ele próprio define. Um agente que pudesse acrescentar a própria chave teria o sistema inteiro num commit.

**Degradação quando não há chave.** O sistema **MUST** continuar funcionando com a assinatura ausente, e **MUST** degradar fechando:

- `chaos validate`, `search`, `context`, `<tipo> create|update`: funcionam normalmente. Leitura e escrita A0–A2 não dependem de assinatura.
- A3 `out_of_band` e A4: **impossíveis**. Não há caminho de aprovação; a tarefa vai a HND com `blocked_reason: no_signing_key` e espera o humano configurar a chave.
- `chaos health` reporta `drift`, não `broken`: o repositório é utilizável, a autonomia é que está rebaixada.

Um sistema que, sem chave, tratasse todo commit como humano estaria invertendo a falha exatamente como a v2.5 fazia.

**Limites honestos desta garantia.** A assinatura prova que a chave privada foi usada na máquina que a guarda. Não prova que uma pessoa a acionou conscientemente: quem comprometa a máquina do usuário e a frase-secreta assina o que quiser. O que a v2.6 compra é a **redução da superfície** — de "qualquer sessão na nuvem, com dois comandos" para "acesso à máquina do usuário e à frase-secreta". É uma fronteira real, não uma prova absoluta, e descrevê-la como prova absoluta seria repetir o erro que o spike acabou de corrigir.
...
```text
chaos init | validate | status | health       # health: §21.1 (toolchain em execução × vendorizado)
chaos id new <tipo>
...
chaos search "q" [--as-of <data>] | index rebuild
chaos graph build | neighbors <id> [--predicate] | path <a> <b> | subtree <id> [--depth]
chaos area create <slug> | sync [<slug>] | show <slug> | list
...
chaos key new <papel> | register <papel> <arquivo.pub> | verify [<commit>] | list
...
- **`chaos key` gera e registra; não autoriza.** `key new` cria o par na máquina e **nunca** escreve a chave privada no repositório; `key register` prepara a linha do `allowed_signers` e **para aí** — a inclusão é commit em protected path, portanto humana e assinada, pela regra que o próprio arquivo define (§17.6). Um `key register` que comitasse sozinho seria a porta que §17.6 existe para trancar. `key verify` é leitura: responde se um commit verifica e contra qual principal.

### 21.1 Contrato de `chaos health`

`chaos health` responde a uma pergunta só, e é a que ninguém pensa em fazer até um defeito inexplicável aparecer: **o código que está executando é o código que este repositório declara?**

Verifica, e reporta em JSON:

| Verificação | Fonte |
|---|---|
| Tag das ferramentas vendorizadas | `metadata/tooling.yaml.vendored_tag` (protected path, §4.1) |
| Versão e hash do `chaos` e do `order` em execução | o próprio binário/entrypoint |
| Correspondência entre os dois | comparação — é o coração do comando |
| Presença e versão dos componentes opcionais | camada episódica (§4.2), modelos locais, worker |
| Pré-condições de plataforma | normalização de finais de linha ativa, limite de caminho, caracteres reservados (§7.2, §17.1) |

Três estados, com códigos de saída distintos:

- **`ok`** — o toolchain em execução corresponde ao vendorizado; componentes opcionais ausentes não alteram este estado (§4.2: são opcionais em sentido verificável).
- **`drift`** — executa versão diferente da vendorizada. Não é erro fatal: o repositório continua legível e editável, e um humano pode trabalhar. Mas a execução autônoma é rebaixada (ORDER §31), porque reprodutibilidade (ORDER §39) deixou de valer: o RUN registraria uma versão de ferramenta que não é a que agiu.
- **`broken`** — falta ferramenta obrigatória, `tooling.yaml` ausente ou ilegível, ou pré-condição de plataforma violada. Nada autônomo roda.

O comando é **somente leitura**: nunca atualiza, nunca instala, nunca corrige. Alinhar o toolchain é `chaos tooling update <tag>`, que é escrita em protected path e portanto humana. Um `health` que se conserta sozinho seria um caminho de escrita não governado com nome tranquilizador.
...
- **AT-19 Identidade não é prova (v2.3, reescrito na v2.6):** um commit cujo **autor e committer** são a identidade humana, sem trailer algum, mas **sem assinatura verificável**, é classificado como **executor** — não como humano. Nenhuma aprovação A4 que dependa dele é aceita. Este AT reproduz diretamente o teste 5 do spike (§0.0) e falha em qualquer implementação que ainda infira autoria humana de autor, committer ou ausência de trailer.
...
- **AT-27 Ator é atribuição, autorização é assinatura (v2.3, reescrito na v2.6):** `chaos commit` continua obtendo o `Actor:` da credencial Git via `metadata/registries/executors.yaml`, e continua recusando ator ausente do registry — isso é higiene de atribuição. Mas nenhuma decisão de autorização consulta esse arquivo: um commit com `Actor: human:owner`, credencial humana e sem assinatura **não** abre nenhum portão que a assinatura ausente feche. Uma implementação que conceda privilégio com base no registry falha aqui.
...
- **AT-35 Toolchain verificável (v2.5):** com `metadata/tooling.yaml` declarando uma tag e o binário em execução sendo de outra, `chaos health` retorna `drift` com código de saída diferente de zero e nomeia as duas versões; com as versões coincidindo, retorna `ok`. Com `tooling.yaml` ausente, retorna `broken`. Em nenhum dos três casos o comando altera qualquer arquivo — o repositório antes e depois é byte a byte idêntico. Um agente que edite `metadata/tooling.yaml` é negado pelo hook e pelo validador (§4.1).

- **AT-36 Assinatura como portão (v2.6):** escrita em protected path e decisão de APV A4 são **aceitas** quando o commit traz assinatura verificável de chave `human:*` do `allowed_signers`, e **recusadas** em todos os demais casos — sem assinatura, com assinatura de chave de worker, com assinatura de chave ausente do arquivo, e com assinatura que não verifica. Os quatro casos negativos são testados separadamente: uma implementação que apenas confira "existe assinatura" passa no primeiro e falha nos três seguintes.

- **AT-37 Degradação sem chave (v2.6):** num repositório sem nenhuma chave humana registrada, `chaos validate`, `search`, `context`, `<tipo> create|update` e `index rebuild` funcionam integralmente; toda tarefa que exija A4 vai a `blocked(no_signing_key)` e **nenhuma** é executada; `chaos health` retorna `drift`, não `broken`, e não altera arquivo algum. Uma implementação que, sem chave, trate commits como humanos falha aqui — e é a falha exata que a v2.5 tinha por desenho.

- **AT-38 `allowed_signers` é expirável, não apagável (v2.6):** remover uma linha do `allowed_signers` é violação `integrity` detectada pelo validador; acrescentar `valid-before` à mesma linha é aceito, e os commits que aquela chave assinou **antes** da data continuam verificando. Alterar o arquivo em commit não assinado por chave humana é recusado pelo hook e pelo validador — inclusive quando a alteração é o próprio registro da primeira chave humana, cujo caminho é o `chaos init` (§21), fora do alcance de qualquer agente.
...
LLM como banco; vector DB como única memória; Obsidian como dependência; blockchain como banco; PM SaaS como autoridade; arquivos monolíticos; agentes editando views derivadas; decisão crítica sem protocolo; ação irreversível sem aprovação; acoplamento a fornecedor; complexidade distribuída sem necessidade; **log de auditoria regenerado; IDs sequenciais globais; risco auto-declarado por agente; estado de execução vivendo só na sessão**; **camada episódica tratada como fonte; promoção implícita de observação a fato; índice como única cópia de um dado; poda automática de memória**; **identidade Git tratada como prova de autoria; garantia de segurança expressa como ausência de marca em vez de presença; barreira cujo texto de confirmação é redigido pelo agente que ela restringe**.

O último merece registro do caso que o produziu. No spike de §0.0, a plataforma detectou corretamente um trailer de autorização forjado e recusou o commit. Em seguida, o próprio agente restringido ofereceu ao humano três formas de contornar a recusa, redigindo o pedido em linguagem de procedimento de rotina — "isso me permitiria contornar o bloqueio de segurança para completar o comando". Confirmado, o contorno funcionou. Uma barreira que o agente restringido pode dissolver escrevendo ele próprio o texto do consentimento protege contra agente distraído, não contra agente comprometido. Consequência normativa em ORDER §18.1: o texto que o humano lê ao aprovar **MUST** ser gerado pelo motor de risco a partir da ação pretendida, nunca pelo executor que a propõe.

## 32. Definition of Done — CHAOS v2.6

CHAOS v2.6 é compatível quando: schemas formalizados; IDs estáveis no novo formato; Área/Project State reconstruíveis; PMMS validável; decisões rastreáveis; provenance preservada; views regeneráveis; BM25 + grafo funcionam sem vetor; `events.jsonl` append-only verificável; `order/` persiste RUN/APV/HND/SES; `PAUSED` respeitado; Session Protocol descrito em `AGENTS.md`; backup/restore funciona; Obsidian opcional; ORDER externo/substituível; **camada episódica opcional e verificadamente dispensável, com nada canônico existindo apenas nela**; **todo índice reconstruível a partir dos arquivos**; **toolchain em execução verificável contra o vendorizado (`chaos health`)**; **autoria humana provada por assinatura verificável e nunca inferida de autor, committer ou ausência de trailer (§17.6); A4 e escrita em protected path recusados sem assinatura de chave `human:*`; sistema utilizável e verificadamente degradado-fechando quando não há chave**; nenhuma dependência estrutural de fornecedor; **todos os Acceptance Tests de §30 passam** — a suíte inteira, deliberadamente sem faixa numérica: enunciar "AT-01 a AT-NN" faz com que acrescentar um teste sem mover o limite permita declarar conformidade justamente sem o teste que a última revisão julgou necessário.
